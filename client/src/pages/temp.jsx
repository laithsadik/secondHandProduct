...
console.log("xx")
    ...
    const fetchListings = async () => {
      ...
      if (data.length > 2 && pick === "lisintgs") {
        setShowMore(true);
    ...
    const getAllPosts = async () => {
      if (pick === "categories") {
        ...
        await fetchPets();
        if (pick === "categories") {
          if (dataLength > 0 || dataVLength > 0 || dataPLength > 0) {
            console.log("xxx");
            setShowMore(true);
          } else {
            setShowMore(false);
  ...
  useEffect(() => {
    if (
      ...
      console.log("xxx");
      const tempMergedList = [...vehicles, ...listings, ...pets];
  ...
  const onShowMoreClick = async () => {
    ...
    if (pick === "categories") {
      console.log("xxxxx");
      if (startIndex > numberOfVehicles) {
    ...
    <div className="flex flex-col md:flex-row ">
      ...
      <div className="flex-1">
        ...
        <div className="p-7 flex flex-wrap gap-5">
          {!loading && listings.length === 0 && vehicles.length === 0 && pets.length===0 && (
          ...
          {!loading &&
            ...
            mergedList.length > 0 &&
            mergedList.map((post) =>
              (post.vehicletype === undefined && post.pickPet === undefined) ? (
